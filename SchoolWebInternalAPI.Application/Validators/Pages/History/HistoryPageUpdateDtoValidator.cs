using FluentValidation;
using SchoolWebInternalAPI.Application.DTOs.Pages.History;

namespace SchoolWebInternalAPI.Application.Validators.Pages.History
{
    public class HistoryPageUpdateDtoValidator : AbstractValidator<HistoryPageUpdateDto>
    {
        public HistoryPageUpdateDtoValidator()
        {
            // Title (required)
            RuleFor(x => x.Title)
                .NotEmpty().WithMessage("Title is required.")
                .MaximumLength(150).WithMessage("Title cannot exceed 150 characters.");

            // Subtitle
            RuleFor(x => x.Subtitle)
                .MaximumLength(250)
                .When(x => !string.IsNullOrWhiteSpace(x.Subtitle))
                .WithMessage("Subtitle cannot exceed 250 characters.");

            // Content HTML
            RuleFor(x => x.ContentHtml)
                .MaximumLength(10000)
                .When(x => !string.IsNullOrWhiteSpace(x.ContentHtml))
                .WithMessage("Content HTML cannot exceed 10,000 characters.");

            // Image URL
            RuleFor(x => x.SideImageUrl)
                .MaximumLength(255)
                .When(x => !string.IsNullOrWhiteSpace(x.SideImageUrl))
                .WithMessage("Side image URL cannot exceed 255 characters.")
                .Must(BeValidUrl)
                .When(x => !string.IsNullOrWhiteSpace(x.SideImageUrl))
                .WithMessage("Side image URL must be a valid absolute URL.");

            // SEO Title
            RuleFor(x => x.SeoTitle)
                .MaximumLength(150)
                .When(x => !string.IsNullOrWhiteSpace(x.SeoTitle))
                .WithMessage("SEO title cannot exceed 150 characters.");

            // SEO Description
            RuleFor(x => x.SeoDescription)
                .MaximumLength(300)
                .When(x => !string.IsNullOrWhiteSpace(x.SeoDescription))
                .WithMessage("SEO description cannot exceed 300 characters.");

            // Open Graph Image
            RuleFor(x => x.OgImageUrl)
                .MaximumLength(255)
                .When(x => !string.IsNullOrWhiteSpace(x.OgImageUrl))
                .WithMessage("OG Image URL cannot exceed 255 characters.")
                .Must(BeValidUrl)
                .When(x => !string.IsNullOrWhiteSpace(x.OgImageUrl))
                .WithMessage("OG Image URL must be a valid absolute URL.");
        }

        private bool BeValidUrl(string url)
        {
            return Uri.TryCreate(url, UriKind.Absolute, out _);
        }
    }
}
